...
const ColAssetsCards: React.FC<cardProps> = ({ data }) => {
  ...
  return (
    <>
      <Link
        className="flex min-w-[1216px] w-full justify-between items-center bg-neutral500 bg-opacity-50 hover:bg-neutral400 hover:bg-opacity-30 rounded-2xl pr-8 pt-4 pb-4 pl-4 "
      ...
      >
        ...
        <div className="flex justify-end items-center w-full text-start">
          <div className="w-[200px] grid gap-1">
          ...
          >
            <span className="font-medium text-lg flex justify-start text-neutral50">
              <span
              ...
              {(data.price ?? 0) > 0 && (
                <span className="hidden group-hover:block lg:absolute lg:-top-5 Lg:left-12 text-white bg-white bg-opacity-25 py-2 px-5 rounded-lg cursor-pointer transition-all duration-300 ease-in-out">
                  List
                </span>
              )}
            </span>
